                    ...
                    <ul className='list-unstyled text-capitalize fs-4'>
                        {
                            ShopsCategoryLists.map((elm, i) => {
                                return (
                                    <>
                                        <li >
                                            <span onClick={() => setAtomHandle({ ...atomHandle, category_id: elm.id })} className='list_head' key={i}>{elm.name}</span>
                                                ...
                                                <li className='ps-1'>
                                                    {
                                                        elm.subcat.map((elem, j) => {
                                                            return (
                                                                <>
                                                                    <span key={j} onClick={() => setAtomHandle({ ...atomHandle, category_id: elem.id })} className='sublist_head text-muted fw-bolder'> {elem.name}</span>
                                                                    <ul className=' list-unstyled ps-2 pt-1  fs-6'>
                                                                        {
                                                                            elem.subcat.map((elsd, k) => {
                                                                                return (
                                                                                    <>
                                                                                        <li key={k} className='p-1 text-muted'><span className='subSubHead' onClick={() => setAtomHandle({ ...atomHandle, category_id: elsd.id })}>{elsd.name}</span></li>
                                                                                    </>
                                                                                )
                                                                            })
                                                                        }
